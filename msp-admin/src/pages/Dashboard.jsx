...
const StatCard = ({ icon, label, value, to }) => (
  <Link to={to} className="bg-zinc-900 rounded-2xl p-5 border border-zinc-800 hover:border-zinc-700 transition flex items-center gap-4">
    <div className="w-11 h-11 bg-zinc-800 rounded-xl flex items-center justify-center shrink-0">{icon}</div>
    <div>
  ...
  return (
    <div className="p-8 space-y-8">
      ...
      {analytics?.followUpNeeded > 0 && (
        <Link to="/jobs" className="flex items-center gap-3 bg-zinc-900 border border-zinc-700 rounded-2xl px-5 py-4 hover:border-zinc-600 transition">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-500">Applied 7+ days ago with no response — open Job Tracker to review</p>
          </div>
        </Link>
      )}
      ...
      <section>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard icon={<Briefcase size={18} className="text-zinc-400" />} label="Total" value={jobStats?.total} to="/jobs" />
          <StatCard icon={<TrendingUp size={18} className="text-zinc-400" />} label="Interviews" value={interviewCount} to="/jobs" />
          <StatCard icon={<Clock size={18} className="text-zinc-400" />} label="Applied / Awaiting" value={jbs.applied || 0} to="/jobs" />
          <StatCard icon={<CheckCircle size={18} className="text-zinc-400" />} label="Offers" value={(jbs.offer || 0) + (jbs.accepted || 0)} to="/jobs" />
        </div>
      </section>
                ...
                {analytics.weeklyData.map((w, i) => {
                  ...
                  return (
                    <div key={i} className="flex-1 flex flex-col items-center gap-1">
                      ...
                      <div className="w-full bg-zinc-600 rounded-t transition-all" style={{ height: `${h}%`, minHeight: w.count ? 4 : 0 }} />
